...
import { 
  ...
  Users,
  ArrowLeft,
  ChevronDown, // Import icon baru
  ChevronUp    // Import icon baru
} from 'lucide-react';
  ...
  const fetchUserStats = async () => {
    ...
    if (user) {
      ...
      if (stats) {
        ...
        setLevelData(info);
        setReferralCode(stats.referral_code || "Generating...");
        ...
        <div className="px-6 mb-4">
           <div className="bg-gradient-to-r from-[#2B3139]/50 to-[#1E2329] border border-[#2B3139] rounded-xl p-3 flex items-center justify-between relative overflow-hidden group hover:border-[#FCD535]/30 transition-all">
              ...
              <button 
                ...
                title="Copy Code"
              >
                 {copyFeedback ? <span className="text-[9px] font-bold">COPIED</span> : <Copy size={14} />}
              </button>
           </div>
        </div>
