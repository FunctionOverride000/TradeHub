...
export default function CreateArenaPage() {
  ...
  // 1. Cek Auth Session & Fetch Level
  useEffect(() => {
    const init = async () => {
      if (!supabase) return;
      ...
      // Ambil Level Creator untuk Diskon
  ...
  // 2. Hitung Diskon Fee Pembuatan Berdasarkan Level
  ...
  // 3. Kalkulasi Total Biaya (Fee Pembuatan Diskon + Biaya Premium + Deposit Reward)
  const totalCost = React.useMemo(() => {
    let cost = discountedFee;
    
    // Biaya tambahan fitur premium (tidak didiskon)
  ...
  const executeRealPayment = async (amount: number): Promise<{ signature: string, wallet: string } | null> => {
    // Jika total biaya 0 (misal level tinggi & reward 0 & fitur basic), skip payment
    if (amount <= 0) {
    ...
    try {
      ...
      if (err.message && (err.message.includes("disconnected port") || err.message.includes("connection not established"))) {
         setErrorMsg("Koneksi Wallet Terputus. Mohon Refresh Halaman (F5).");
      } else {
    ...
    try {
      ...
      const { error } = await supabase.from('rooms').insert([
        { 
          ...
          creator_id: user.id,
          creator_wallet: paymentResult.wallet === 'system' ? user.email : paymentResult.wallet, // Fallback jika free
          start_time: new Date(formData.start_date).toISOString(),
          end_time: new Date(formData.end_date).toISOString(),
          
          reward: `${rewardVal} SOL`, 
          
          // Data untuk Robot Distribusi
