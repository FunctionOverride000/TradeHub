...
// AUDIT FIX: Gunakan RPC Private & Treasury via Env Var
...
export default function ArenaDetailPage() {
  ...
  useEffect(() => {
    ...
    if (SOLANA_RPC.includes('mainnet-beta.solana.com')) {
         console.warn("⚠️ WARNING: Using Public Solana RPC. Monitoring may be slow.");
    }
  ...
  const handleJoinArena = async () => {
    ...
    try {
      ...
      // AUDIT FIX: Ganti direct Insert dengan RPC 'join_arena'
      // Ini mencegah Race Condition & Bypass Kuota
      const { data: joinResult, error: rpcError } = await supabase.rpc('join_arena', {
        p_room_id: arenaId,
        p_user_id: user.id,
      ...
      if (rpcError) {
        // Error jaringan/RPC Supabase
        console.error("RPC Error:", rpcError);
        throw new Error("System Error: Failed to contact registration server.");
      }

      // Handle Logic Error dari Database (misal: Penuh, Waktu Habis)
      if (joinResult && !joinResult.success) {
         throw new Error(joinResult.message || "Failed to join arena.");
      }
