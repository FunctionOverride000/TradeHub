...
// --- KONFIGURASI ---
// Pastikan Anda sudah set env var ini: npx supabase secrets set ALCHEMY_SOLANA_URL=...
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`🔍 Memeriksa ${activeParticipants.length} peserta aktif...`)
    ...
    for (const participant of activeParticipants) {
      ...
      for (const sigInfo of signatures) {
        ...
        if (balanceChange > 5000) {
          ...
          // Jika Swap, biasanya melibatkan program Raydium/Jupiter/Pump.fun
          ...
          // Jika ini System Transfer DAN tidak ada program DEX (logika sederhana)
          // Untuk keamanan maksimal: Anggap semua System Transfer masuk sebagai deposit
          // Karena DEX swap biasanya tercatat sebagai 'inner instruction', bukan top level instruction system transfer murni.
